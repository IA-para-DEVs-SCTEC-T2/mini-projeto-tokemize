...
Cobre: scanner, analyzer, selector, summarizer, generator e reporter.
"""
...
def test_scan_repository_raises_not_a_directory_error():
    """NotADirectoryError para caminho inválido."""
...
def test_property_8_scanner_total_files_consistent(repo_path: str):
    """Property 8: total_files é consistente com files."""
...
@given(
    invalid_path=st.one_of(
        ...
        st.text(min_size=1, max_size=50).filter(lambda p: not p.startswith(".")),
    )
)
def test_property_9_scanner_raises_for_invalid_paths(invalid_path: str):
    """Property 9: NotADirectoryError para caminhos inválidos."""
    import os
...
def _make_analysis_output(n: int) -> AnalysisOutput:
    """Helper: cria AnalysisOutput com n arquivos."""
    files = [
        AnalyzedFile(
            path=f"file{i}.py",
            language="python",
    ...
    return AnalysisOutput(analyzed_files=files, total_analyzed=n, total_skipped=0)


def test_select_relevant_ordered_by_score():
    """selected_files ordenados por relevance_score decrescente."""
    output = select_relevant(_make_analysis_output(3), "task")
    for i in range(len(output.selected_files) - 1):
        assert output.selected_files[i].relevance_score >= output.selected_files[i + 1].relevance_score


def test_select_relevant_task_preserved():
    """SelectionOutput.task == task passado como argumento."""
    task = "add authentication"
    output = select_relevant(AnalysisOutput(), task)
...
def test_property_11_selector_ordered_by_relevance(num_files: int):
    """Property 11: arquivos ordenados por relevância decrescente."""
    output = select_relevant(_make_analysis_output(num_files), "task")
...
def test_property_12_selector_task_preserved(task: str):
    """Property 12: task preservada no output."""
    output = select_relevant(AnalysisOutput(), task)
    assert output.task == task
...
def test_generate_prompt_fallback_to_task_when_empty_context():
    ...
    output = generate_prompt(SummaryOutput(summarized_content="", token_count=0, files_summarized=0), "add tests")
    assert "add tests" in output.prompt
...
def test_generate_prompt_contains_context_and_task():
    ...
    output = generate_prompt(
        SummaryOutput(summarized_content="file1.py\nfile2.py", token_count=2, files_summarized=2),
        "refactor auth",
    )
...
def test_property_13_generator_fallback_to_task(task: str):
    """Property 13: task está presente no prompt gerado."""
    output = generate_prompt(SummaryOutput(), task)
...
def test_format_result_success_with_prompt():
    ...
    result = format_result(GeneratorOutput(prompt="final prompt", token_count=2))
...
def test_format_result_success_with_empty_prompt():
    ...
    result = format_result(GeneratorOutput(prompt="", token_count=0))
    assert result.success is True
...
def test_format_result_never_raises():
    ...
    for gen_output in [
        ...
        GeneratorOutput(prompt="normal", token_count=1),
    ]:
...
def test_property_14_reporter_never_raises(prompt: str, token_count: int):
    ...
    result = format_result(GeneratorOutput(prompt=prompt, token_count=token_count))
    assert result.success is True
...
def test_property_15_reporter_round_trip_prompt(prompt: str):
    """Property 15: round-trip do prompt."""
    result = format_result(GeneratorOutput(prompt=prompt, token_count=len(prompt.split())))

...
etapa do pipeline: scanner → analyzer → selector → summarizer → generator → reporter.
"""
...
class SelectedFile:
    ...
    Attributes:
        ...
        relevance_score: Score de relevância no intervalo ``[0.0, 1.0]``.
    """
...
@dataclass
class SummaryOutput:
    """Resultado da etapa de sumarização de contexto.

    Attributes:
        summarized_content: Conteúdo resumido dos arquivos selecionados.
        token_count: Estimativa do número de tokens em ``summarized_content``.
        files_summarized: Total de arquivos incluídos em ``summarized_content``.
    """
...
class GeneratorOutput:
    """Resultado da etapa de geração do context pack.

    Attributes:
        prompt: Context pack final formatado em Markdown.
...
class PipelineResult:
    ...
    Attributes:
        ...
        prompt: Context pack final gerado pelo pipeline. Vazio em caso de falha.
...
class CachedContext:
    ...
    Attributes:
        task_description: Descrição da tarefa original.
        content: Conteúdo final a ser utilizado pelo agente.

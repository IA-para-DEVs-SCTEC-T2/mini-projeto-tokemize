...
Os módulos `analyze_repository`, `select_relevant_artifacts`, `compress_context`, `save_context`, `build_prompt` já foram implementados por outros membros da equipe.

## Sua responsabilidade

Você é responsável por:
1. `src/tokemize/integrations/clipboard/clipboard.py` — criar novo
2. `src/tokemize/integrations/clipboard/__init__.py` — criar novo
3. `src/tokemize/cli.py` — criar (mover de `cli.py` na raiz para dentro do pacote)

---

## 1. `src/tokemize/integrations/clipboard/clipboard.py` — arquivo novo
...
## 2. `src/tokemize/integrations/clipboard/__init__.py` — arquivo novo

Crie este arquivo:

```python
"""Integração com a área de transferência do sistema operacional."""

from tokemize.integrations.clipboard.clipboard import ClipboardError, copy_to_clipboard
...
## 3. `src/tokemize/cli.py` — arquivo novo

**Atenção:** O arquivo `cli.py` deve ser criado em `src/tokemize/cli.py`, **não** na raiz do projeto. Se existir um `cli.py` na raiz, ele pode ser removido após este arquivo ser criado.

Crie `src/tokemize/cli.py` com o seguinte conteúdo:
...
def _run_pipeline(
    ...
    artifacts = _run_step("Intelligent_Selector", select_relevant_artifacts, file_analyses, task_description)
    ...
    prompt = _run_step("Prompt_Builder", build_prompt, compressed_ctx, task_description, context_file_path)
    ...
    if clipboard_ok:
        typer.echo("✅ Prompt otimizado gerado e copiado para a área de transferência. Cole agora no chatbot da sua IDE.")
    else:
